...
class GoogleMapsServices {
    ...
    func getRoutes(_ origin: CLLocationCoordinate2D, destination: CLLocationCoordinate2D, completion: @escaping ([Routes]) -> ()) {
        let url = "https://maps.googleapis.com/maps/api/directions/json?origin=34.062469,-118.305718&destination=\(origin.latitude),\(origin.longitude)&mode=driving&key=AIzaSyDSiFR_IXzPLoKoQnzDcPavcAGThhmW55M"
